...
def draw_opt_difference_in_one_compiler(compiler):
    opt_difference_file = "opt_difference.json"
    opt_difference = read_json(opt_difference_file)
    # box_labels = ["O0-O1", "O0-O2", "O0-O3", "O1-O2", "O1-O3", "O2-O3"]
    ...
    ax.set_ylabel("Node Similarity", fontsize=16)
    plt.subplots_adjust(bottom=0.25, left=0.15)
    plt.savefig(compiler + "_node_difference.png")
    plt.show()


if __name__ == '__main__':
    draw_opt_difference_in_one_compiler("clang")
    draw_opt_difference_in_one_compiler("gcc")
    # draw_opt_difference()
