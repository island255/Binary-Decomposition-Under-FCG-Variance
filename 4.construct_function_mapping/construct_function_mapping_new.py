            ...
            for compiler in binary_to_fcg[binary_name][arch]:
                ...
                for index1, opt1 in enumerate(opt_list):
                    for index2 in range(index1 + 1, len(opt_list)):
                        ...
                        if opt1 not in opt_nodes or opt2 not in opt_nodes:
                            continue
...
def main():
    ...
    write_json("/data1/jiaang/binkit2/4.construct_function_mapping/6_opts/binary_to_fcg.json", binary_to_fcg)

    opt_difference, node_neighbor_difference = analyze_opt_difference(binary_to_fcg)
    write_json("/data1/jiaang/binkit2/4.construct_function_mapping/6_opts/opt_difference.json", opt_difference)
    write_json("/data1/jiaang/binkit2/4.construct_function_mapping/6_opts/node_neighbor_difference.json", node_neighbor_difference)
