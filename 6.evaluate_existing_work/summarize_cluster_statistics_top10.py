...
def analyze_opt(result_file):
    ...
    if "_vs_" in result_file:
        binary_name1 = result_file.split("_vs_")[0]
        binary_name2 = result_file.split("_vs_")[1]
    else:
...
def summarize_cluster_statistics():
    ...
    evaluate_method = "BMVul"
    ModX_evaluate_results_dir = r"/data1/jiaang/binkit2/6.evaluate_existing_work/"+ evaluate_method + "/results_top10"
    opt2opt_statistics = {}
    for binary_name in os.listdir(ModX_evaluate_results_dir):
        ...
        for result_file in os.listdir(binary_dir):
            ...
            opt2opt_statistics = add_statistics(opt2opt_statistics, result)
    write_json("top10_x86_64/" + evaluate_method + "_opt2opt_statistics.json", opt2opt_statistics)
